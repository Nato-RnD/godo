...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"zopenerp":"Godo","action":"Partners"}');
        await click(webClient.el.querySelector(".o_data_row"));
        await legacyExtraNextTick();
        assert.strictEqual(titleService.current, '{"zopenerp":"Godo","action":"First record"}');
        await click(webClient.el.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"zopenerp":"Godo","action":"Second record"}');
    });
