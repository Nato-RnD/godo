...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Godo" });
    assert.strictEqual(env.services.title.current, "Godo");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Godo", chat: null });
    assert.strictEqual(env.services.title.current, "Godo");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Godo - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Godo", action: "Import" });
    assert.strictEqual(env.services.title.current, "Godo - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Godo", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Godo - Import"); // parts is a copy!
});
