...
QUnit.test('rendering with Smodo Android has a request (default)', async function (assert) {
    assert.expect(4);
    ...
    assert.ok(
        ...
        "should display a notification counter next to the messaging menu for Smodo Android request"
    );
    assert.strictEqual(
        document.querySelector('.o_MessagingMenu_counter').textContent,
        "1",
    ...
    assert.strictEqual(
        ...
        'Smodo Android has a request',
        "notification should display that Smodo Android has a request"
    );
});

QUnit.test('rendering without Smodo Android has a request (denied)', async function (assert) {
    assert.expect(2);
...
QUnit.test('rendering without Smodo Android has a request (accepted)', async function (assert) {
    assert.expect(2);
