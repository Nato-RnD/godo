...
# here to init the T1 fonts cache at the start-up of Godo so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at Godo start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Godo with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "Godo is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
